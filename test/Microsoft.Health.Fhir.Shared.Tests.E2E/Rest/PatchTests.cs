...
namespace Microsoft.Health.Fhir.Tests.E2E.Rest
{
    ...
    public class PatchTests : IClassFixture<HttpIntegrationTestFixture>
    {
        ...
        [Fact]
        [Trait(Traits.Priority, Priority.One)]
        public async Task GivenAServerThatSupportsIt_WhenSubmittingReplaceBirthdatePatch_ThenServerShouldPatchCorrectly()
        {
            var poco = Samples.GetDefaultPatient().ToPoco<Patient>();
            FhirResponse<Patient> response = await _client.CreateAsync(poco);
            Assert.NotNull(response.Resource.Address);
        ...
        [Fact]
        [Trait(Traits.Priority, Priority.One)]
        public async Task GivenAServerThatSupportsIt_WhenSubmittingRemovingBirthdatePatch_ThenServerShouldPatchCorrectly()
        {
            var poco = Samples.GetDefaultPatient().ToPoco<Patient>();
            FhirResponse<Patient> response = await _client.CreateAsync(poco);
            string patchDocument =
        ...
        [Fact]
        [Trait(Traits.Priority, Priority.One)]
        public async Task GivenAServerThatSupportsIt_WhenSubmittingAAddNestedPropertyPatch_ThenServerShouldPatchNewPropertyCorrectly()
        {
            var poco = Samples.GetDefaultPatient().ToPoco<Patient>();
            poco.Address.Clear();
            FhirResponse<Patient> response = await _client.CreateAsync(poco);
        ...
        [Fact]
        [Trait(Traits.Priority, Priority.One)]
        public async Task GivenAServerThatSupportsIt_WhenSubmittingAReplaceNestedPropertyPatch_ThenServerShouldPatchNewPropertyCorrectly()
        {
            var poco = Samples.GetDefaultPatient().ToPoco<Patient>();
            poco.Address.Clear();
            FhirResponse<Patient> response = await _client.CreateAsync(poco);

            string patchDocument = "[{\"op\":\"replace\",\"path\":\"/communication/0/preferred\",\"value\":\"female\"}]";

            using FhirResponse<Patient> patch = await _client.PatchAsync(response.Resource, patchDocument);

            Assert.Equal(HttpStatusCode.OK, patch.Response.StatusCode);
            Assert.Single(patch.Resource.Communication);
            Assert.Equal(false, patch.Resource.Communication[0].Preferred);
        }

        [Fact]
        [Trait(Traits.Priority, Priority.One)]
        public async Task GivenAServerThatSupportsIt_WhenSubmittingARemoveNestedPropertyPatch_ThenServerShouldPatchNewPropertyCorrectly()
        {
            var poco = Samples.GetDefaultPatient().ToPoco<Patient>();
            poco.Address.Clear();
            FhirResponse<Patient> response = await _client.CreateAsync(poco);
            ...
            Assert.Equal(HttpStatusCode.OK, patch.Response.StatusCode);
            Assert.Null(patch.Resource.Communication);
        }
    }
}
